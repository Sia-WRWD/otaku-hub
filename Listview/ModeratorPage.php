...
<head>
    ...
    <link rel="stylesheet" href="ModeratorPage.css">
    <title>OtakuPage| Moderators List</title>
</head>
...
?>
    ...
    <br>
     ...
     <body>
        ...
        <div class="title"><br><h1>Moderators List</h1></div>
        <div class="modtable">

        <form>
        <label for="search-mod-name"><b>Insert Username to Search Moderators<b></label><br>
        <input type="search-mod-name" placeholder="Insert FBI Codename" name="search-mod-name"><br>
    ...
    if(isset($_GET['search-mod-name'])){

        $sql = 'SELECT * FROM moderator WHERE ModFBICodename LIKE "%'.$_GET['search-mod-name'].'%"';
        ...
        if($resultrows > 0){
                ...
                while($row = mysqli_fetch_assoc($result)){
                    ...
                    if ($rowImg['ImgStatus'] == 0){
                          echo '<img src="..\UploadedFiles\profile'.$row['ModPosition'].''.$row['ModID'].'.jpg?' .mt_rand(). '" alt="Profile_Image" /><h2 style="background-color:white;">'.$row['ModFBICodename'];
                      } else {
                          echo '<img src="..\UploadedFiles\profiledefault.jpg" alt="Profile_Image" /><h2 style="background-color:white;">'.$row['ModFBICodename'];
                      }
                    }
                    echo '</td>';
                ...
                echo '<tr/>';
                echo '<tr>';
                echo '<td><h2 style="background-color:white; padding-right: 10px; padding-left: 10px; padding-top: 10px; padding-bottom: 10px;">Oh no! The Moderator You Are Looking For Cannot Be Found. Please Try Again :^(</h2></td>';
                echo '<tr/>';
