...
if(isset($_POST["new-submit"]))  {
    ...
    if (empty($password) || empty($repassword)) {
        header("Location: ../../loginsystem/user/new.php?newpwd=empty");
        exit();
    } else if ($password != $repassword) {
        header("Location: ../../loginsystem/user/new.php?newpwd=repwdnotsame");
        exit();
    }
            ...
            if ($tokenCheck === false) {
              ...
              if (!$row = mysqli_fetch_assoc($result)) {
                ...
                if (!mysqli_stmt_prepare($stmt, $sql)) {
                    ...
                    mysqli_stmt_bind_param($stmt, "s", $tokenEmail);
                    mysqli_stmt_execute($stmt);
                    header("Location: ../../loginsystem/user/login.php?newpwd=passwordupdated");
                } 
