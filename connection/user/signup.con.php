...
if(isset($_POST['signup-submit'])) {
    ...
    if (empty($username) || empty($email) || empty($password) || empty($passwordRepeat)) {
        header("Location: ../../loginsystem/user/signup.php?error=emptyfields&uname=".$username."&email=".$email);
        exit();
    }
      //Check for an invalid username AND email.
      else if (!preg_match("/^[a-zA-Z0-9]*$/", $username) && !filter_var($email, FILTER_VALIDATE_EMAIL)) {
        header("Location: ../../loginsystem/user/signup.php?error=invaliduname&email");
        exit();
      }
      // Check for an invalid username (ONLY letters and numbers.)
      else if (!preg_match("/^[a-zA-Z0-9]*$/", $username)) {
        header("Location: ../../loginsystem/user/signup.php?error=invaliduname&email=".$email);
        exit();
      }
      // Check for an invalid email
      else if (!filter_var($email, FILTER_VALIDATE_EMAIL)) {
        header("Location: ../../loginsystem/user/signup.php?error=invalidemail&uname=".$username);
        exit();
      }
      // Check if the repeated password is NOT the same.
      else if ($password !== $passwordRepeat) {
        header("Location: ../../loginsystem/user/signup.php?error=passwordcheck&uname=".$username."&email=".$email);
        exit();
      }
      else {
        ...
        if (!mysqli_stmt_prepare($stmt, $sql)) {
          ...
          header("Location: ../../loginsystem/user/signup.php?error=sqlerror");
          exit();
        }
        else {
          ...
          if ($resultCount > 0) {
            header("Location: ../../loginsystem/user/signup.php?error=usertaken&email=".$email);
            exit();
          }
          else {
            // Insert Statement + PlaceHolders
            $sql = "INSERT INTO users (Username, UserEmail, UserPassword, UserRealName, 
            UserDescription, UserAge, UserCountry, UserPosition) VALUES (?, ?, ?, ?, ?, ?, ?, '$position');";
            ...
            if (!mysqli_stmt_prepare($stmt, $sql)) {
              ...
              header("Location: ../../loginsystem/user/signup.php?signup=success");
              exit(); 
            } else {
      ...
      header("Location: ../../loginsystem/user/signup.php");
      exit();
    }